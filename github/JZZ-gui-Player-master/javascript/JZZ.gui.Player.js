(function(global, factory) {
  ...
  function Player(x, y) {
    ...
    if (typeof x == 'object')
      for (var k in arg)
        if (arg.hasOwnProperty(k) && typeof x[k] != 'undefined') arg[k] = x[k];
    ...
    if (this._player) {
      ...
      if (this._out || !this._conn) {
        ...
        JZZ().openMidiOut(self._ports).and(function() {
          ...
          if (self._playing) {
            self._player.resume();
            self._moving = setInterval(function() {
              self._move();
            }, 100);
          }
        });
  ...
  Player.prototype._selectMidi = function() {
    ...
    var port = JZZ().openMidiOut(this._newname).or(function() {
      ...
      if (self._out) {
        if (self._playing)
          for (var c = 0; c < 16; c++) self._out._receive(JZZ.MIDI.allSoundOff(c));
